...
// dao = merkezi olmayan banka, akıllı sözleşme işlemleri adil ve şeffaf şekilde yönetmeye yarıyor.

import Trie "mo:base/Trie"; // veri yapısı anahtar ve değer çifti saklar
import Principal "mo:base/Principal";  // hata durumu, genelde metin türlerde ayrıntıları içerir
import Option "mo:base/Option"; //işlemin sonucunu temsil eder, başarılı ise #ok, başarısız ise #err
import Iter "mo:base/Iter";
import Nat "mo:base/Nat";  // koleksiyonun elemanları üzerinde dolaşan döngü
import Result "mo:base/Result";  // bir varlığı veya yokluğu temsil ediyor
import Error "mo:base/Error"; // dfinity nin Internet Comp ağında kullanıcısı, idler benzersiz
...
import List "mo:base/List"; // zaman, geçerli zamanı almak, zaman aralıklarını ölçmek
import Time "mo:base/Time";  // elemanları üzerinde dolaşılabilen bir koleksiyon
...
shared actor class DAO(init : Types.BasicDaoStableStorage) = Self {
    ...
    /// Aramayı yapanın hesabından başka bir hesaba token aktar
    public shared({caller}) func transfer(transfer: Types.TransferArgs) : async Types.Result<(), Text> {
     ...
     /// Aramayı yapanın hesap bakiyesini döndür.
    ...
    /// Bir öneri sun
    ///
    ///Bir teklif bir canister ID, yöntem adı ve yöntem argümanlarını içerir.
    ///Yeterli sayıda kullanıcı teklife "evet" oyu verirse, belirtilen yöntem belirtilen canister üzerinde belirtilen yöntem argümanları ile çağrılacaktır
        ...
        case (?proposal) {
                 ...
                 case (?{ amount_e8s = voting_tokens }) {
                          ...
                          if (votes_yes >= system_params.proposal_vote_threshold.amount_e8s) {
                              // Teklif kabul edildiğinde teklif depozitosunu iade et
                              ignore do ? {
    ...
    ///
    /// Yalnızca teklif yürütme yoluyla çağrılabilir.
    ...
    /// Teklif gönderim depozitosunu arayanın hesabından kes
    ...
    /// Kabul edilen tüm teklifleri gerçekleştir
    func execute_accepted_proposals() : async () {
        ...
        // Teklif durumunu güncelle, böylece bir sonraki kalp atışında alınmayacak şekilde ayarla
